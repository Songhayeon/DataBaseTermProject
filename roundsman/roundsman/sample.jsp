...
<body>
	...
	<%
		...
		<td><%=d_day%></td>
		<td><%=start_date%></td>
		<td><a href="update.jsp?id=<%=serial_number %>&<%=c_id %>">수정</a></td>
		<td><a href="delete.jsp?id=<%=serial_number %>&<%=c_id%>">삭제</a></td>
		</tr>
		<%
		}	%>
		</table>
		...
		{
			query = "select i.name,i.price,i.categorize, c.name, c.phone, c.address_gu,c.address_dong,c.address_detail,d.quantity, d.d_day, d.start_date from delivery d,item i, customer c, roundsman r where r.r_id=? AND i.name=? AND d.r_id=r.r_id AND d.c_id=c.c_id AND d.serial_number=i.serial_number";
		}
		else if(category.equals("c.name"))
		{
			query = "select i.name,i.price,i.categorize, c.name, c.phone, c.address_gu,c.address_dong,c.address_detail,d.quantity, d.d_day, d.start_date from delivery d,item i, customer c, roundsman r where r.r_id=? AND c.name=? AND d.r_id=r.r_id AND d.c_id=c.c_id AND d.serial_number=i.serial_number";
		}
		else if(category.equals("c.dong"))
		{
			query = "select i.name,i.price,i.categorize, c.name, c.phone, c.address_gu,c.address_dong,c.address_detail,d.quantity, d.d_day, d.start_date from delivery d,item i, customer c, roundsman r where r.r_id=? AND c.address_dong=? AND d.r_id=r.r_id AND d.c_id=c.c_id AND d.serial_number=i.serial_number";
		}
